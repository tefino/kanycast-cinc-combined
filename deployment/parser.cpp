...
int Parser::getGlobalDomainParameters() {
    try {
        dm->interval = cfg.lookup("INTERVAL");
        //cout << "BLACKADDER_ID_LENGTH: " << ba_id_len << endl;
    } catch (const SettingNotFoundException &nfex) {
        cerr << "mandatory option INTERVAL is missing" << endl;
        return -1;
    }
    try {
        dm->cache_degree = cfg.lookup("CACHE_DEGREE");
        //cout << "BLACKADDER_ID_LENGTH: " << ba_id_len << endl;
    } catch (const SettingNotFoundException &nfex) {
        cerr << "mandatory option CACHE_DEGREE is missing" << endl;
        return -1;
    }
    try {
        dm->popthreshold = cfg.lookup("POPTHRESHOLD");
        //cout << "BLACKADDER_ID_LENGTH: " << ba_id_len << endl;
    } catch (const SettingNotFoundException &nfex) {
        cerr << "mandatory option POPTHRESHOLD is missing" << endl;
        return -1;
    }
    try {
