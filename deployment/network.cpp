...
void Domain::writeClickFiles(bool montoolstub) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        click_conf << "CACHE_DEGREE      " << cache_degree << "," << endl ;
        click_conf << "POPTHRESHOLD      " << popthreshold << "," << endl ;
        click_conf << "INTERVAL          " << interval << ");" << endl ;
        ...
        /*cinc add new connections*/
        if(unique_ifaces.size() >1)
        {
            cerr<<"Right now only work in single interface scenario"<<endl ;
        }
        click_conf<<"proxy[3]->[2]fw;"<<endl ;
        click_conf<<"classifier0[1]->[3]fw;"<<endl ;
        click_conf<<"proxy[4]->[4]fw;"<<endl ;
        click_conf<<"classifier0[2]->[5]fw;"<<endl ;
        click_conf<<"proxy[5]->[6]fw;"<<endl ;
        click_conf<<"fw[2]->[3]proxy;"<<endl ;
        click_conf<<"fw[3]->[4]proxy;"<<endl ;
        click_conf<<"fw[4]->[0]cacheunit;"<<endl ;
        click_conf<<"fw[5]->[5]proxy;"<<endl ;
        click_conf<<"cacheunit[0]->[0]fw;"<<endl ;
        click_conf<<"classifier0[3]->[7]fw;"<<endl ;
