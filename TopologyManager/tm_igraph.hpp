...
public:
    ...
    /**@brief cinc: calculate the # of nodes in each area
     */
    void calculateNodeNumArea() ;
    /**@brief cinc: multicast path calculation
     */
    void calculateMulticastFID(string &source, vector<string> &destinations, Bitvector &resultFID) ;
    /**@brief cinc: core function for rendevzous
     */
    void get_FID2crouter(set<string>& crouters, set<string> &publishers, string &subscriber, map<string, pair<Bitvector, unsigned int> > &allfids, map<string, Bitvector>& p2sfids) ;
     ...
     */
    string mode;
    /**@brief cinc: # of nodes in each area
     */
    map<string, int> nodeID_num ;
    /**@brief cinc: # of area
     */
    int num_area ;
    /**@brief cinc: all the area
     */
    vector<string> areas ;
    /**@brief all the node IDs*/
    vector<string> allnodeIDs ;
    /**@brief all the router IDs*/
    vector<string> routerIDs ;
    /**@brief area--FID to all the nodes in the area--# of routers in this area*/
    map<string, pair<Bitvector, int> > area_routernum ;
};
