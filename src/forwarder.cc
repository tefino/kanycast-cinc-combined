        ...
        if (pushLocally) {
            if (gc->use_mac) {
                ...
                if(in_port == 11)
                {
                    p->pull(14 + FID_LEN);
                    payload = p->uniqueify();
                    memcpy(payload->data()+sizeof(unsigned char)+NODEID_LEN, &noofhop, sizeof(noofhop)) ;
                    memcpy(payload->data()+sizeof(unsigned char)+NODEID_LEN+sizeof(noofhop), reverse_FID._data, FID_LEN) ;
