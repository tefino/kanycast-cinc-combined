...
public:
    ...
    /**@brief kc: the difference from the original method is that in kc scopes are assigned another property*/
    unsigned int kc_publish_root_scope(RemoteHost *_publisher, String &ID, unsigned char &strategy, unsigned char);
    /**@brief kc: same as the above method
     */
    unsigned int kc_publish_inner_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, unsigned char);
    /**@brief kc: ask all the potential content holder check their storage and notify the subscriber
     */
    void kc_rendezvous(unsigned char type, RemoteHostSet& publishers, RemoteHostSet& subscribers, StringSet& cache_router_ID, StringSet& file_scope_ids, ScopeSet& subscope) ;
