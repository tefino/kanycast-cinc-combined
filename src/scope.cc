...
Scope::Scope(unsigned char _strategy, Scope *_father_scope) {
    ...
    is_first = true ;
    cache_router.clear() ;
    scope_type = UNSPECIFY;
}
Scope::Scope(unsigned char _strategy, Scope *_father_scope, unsigned char _scope_type)
{
    request_count = 0 ;
    current_cache = 0 ;
    current_cache_entry = 0 ;
    ...
    if (_father_scope == NULL) {
        ...
        strategy = _strategy;
        isRoot = true;
    } else if(scope_type == CHUNK_LEVEL){
        strategy = _strategy;
        isRoot = false;
        fatherScopes.find_insert(ScopeSetItem(_father_scope));
        _father_scope->childrenScopes.find_insert(ScopeSetItem(this));
...
String Scope::getlastComponent()
{
    return ids.begin()->first.substring(ids.begin()->first.length()-PURSUIT_ID_LEN, PURSUIT_ID_LEN) ;
}
