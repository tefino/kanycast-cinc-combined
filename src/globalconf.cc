...
int GlobalConf::configure(Vector<String> &conf, ErrorHandler *errh) {
    ...
    if (cp_va_kparse(conf, this, errh,
            ...
            "CACHE_DEGREE", cpkM, cpUnsigned, &cache_degree,
            "POPTHRESHOLD", cpkM, cpUnsigned, &popthreshold,
            "INTERVAL", cpkM, cpUnsigned, &interval,
            cpEnd) < 0) {
        return -1;
    }
    char tempchar[NODEID_LEN-AREA_LENGTH-1] ;
    nodeNUM = atoi(tempchar) ;
    ...
    click_chatter("cache_degree: %d\npopularity threshold: %d\ninterval: %d", cache_degree, popthreshold, interval) ;
    //click_chatter("GlobalConf: configured!");
    return 0;
}

int GlobalConf::cinc_hash(String &input, int numofround, Vector<unsigned int> &value)
{
    value.clear() ;
    unsigned int temphash = 0x811C9DC5 ;
    Vector<unsigned int> hash_result ;
    if(input.length() != PURSUIT_ID_LEN)
        return -1 ;//not accept this input
    for(int i = 0 ; i < numofround ; i++)
    {
        bool found = false ;
        fnv1a((const void*) input.c_str(), input.length(), temphash) ;
        for(int n = 0 ; n < hash_result.size() ; n++)
        {
            if(temphash == hash_result[n])
                return 1 ;
        }
        hash_result.push_back(temphash) ;
        for(int n = 0 ; n < value.size() ; n++)
        {
            if( (temphash%num_router+1) == value[n] )
                found = true ;
        }
        if(found)
            continue ;
        value.push_back(temphash%num_router+1) ;
    }
    return 1 ;
}

unsigned int GlobalConf::fnv1a(const void* data, unsigned int numBytes, unsigned int& hash)
{
